...
async function debugFilters() {
  ...
  seasons.forEach((s: any) =>
    console.log(`  - ${s.name} (${s.year}) [${s.status}] - ID: ${s._id}`)
  );
  ...
  divisions.forEach((d: any) =>
    console.log(`  - ${d.name} (${d.ageGroup}, ${d.skillLevel}) - ID: ${d._id}`)
  );
  ...
  teams
    .slice(0, 5)
    .forEach((t: any) =>
      console.log(`  - ${t.name} (${t.shortName}) - ID: ${t._id}`)
    );
  ...
  console.log(
    "📊 Teams with roster divisions:",
    teamsWithRosterDivisions.length
  );
  teamsWithRosterDivisions.slice(0, 3).forEach((team: any) => {
    console.log(`  - ${team.name}:`);
    team.rosters?.forEach((roster: any) => {
      console.log(
        `    Season: ${roster.season?.name}, Division: ${
          roster.division?.name || "None"
        }`
      );
    });
  });
