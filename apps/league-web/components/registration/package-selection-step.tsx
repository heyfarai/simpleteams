...
export function PackageSelectionStep({
  ...
  const packageForPreference = (selectedPackage ||
    "full-season") as PackageType;
  const { isInstallmentEnabled, setInstallmentPreference, isLoading } =
    useInstallmentPreference(packageForPreference, user?.id);
  ...
  const installmentsAvailable = selectedPackage
    ? isInstallmentAvailable(selectedPackage as PackageType)
    : true;
      ...
      <div className="p-6">
        <div className="text-center space-y-6">
          ...
          {paymentMethod === "installments" && installmentsAvailable && (
            <div className="hidden bg-white/70 rounded-lg p-4 border">
              <p className="text-sm text-gray-700 leading-relaxed">
                {selectedPackage &&
                getPackageConfig()[selectedPackage as PackageType]?.installments
                  ? `Split your payment into ${
                      getPackageConfig()[selectedPackage as PackageType]!
                        .installments!.installments
                    } monthly payments of $${(
                      getPackageInstallmentAmount(
                        selectedPackage as PackageType
                      ) / 100
                    ).toFixed(2)} CAD. First payment due today.`
          ...
          {selectedPackage && !installmentsAvailable && (
            <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
              <p className="text-sm text-orange-700">
                Installment payments are not available for this package. Only
