      ...
      <div className="container mx-auto px-4 py-8">
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">The requested season could not be found.</p>
            <Button asChild>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="text-4xl font-bold text-foreground mb-2">{season.name}</h1>
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <Dialog open={isGameDialogOpen} onOpenChange={setIsGameDialogOpen}>
                    ...
                    <DialogContent className="max-w-2xl">
                      ...
                      <GameForm seasonId={season.id} onClose={() => setIsGameDialogOpen(false)} />
                    </DialogContent>
                  </Dialog>
                </CardHeader>
                <CardContent>
                  {sessions.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-muted-foreground mb-4">Create your first session to start scheduling games</p>
                    ...
                    <div className="space-y-6">
                      {sessions.map((session) => {
                        const sessionGames = getAdminGamesBySessionId(session.id)
                        return <SessionScheduleCard key={session.id} session={session} games={sessionGames} />
                      })}
                    </div>
                  )}
                </CardContent>
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <Dialog open={isConferenceDialogOpen} onOpenChange={setIsConferenceDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <ConferenceForm seasonId={season.id} onClose={() => setIsConferenceDialogOpen(false)} />
                    </DialogContent>
                  </Dialog>
                </CardHeader>
                <CardContent>
                  {season.conferences.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-semibold text-foreground mb-2">No conferences yet</h3>
                      <p className="text-muted-foreground mb-4">
                        Create your first conference to organize teams into divisions
                      </p>
...
function ConferenceTreeItem({
  ...
  const totalTeams = conference.divisions.reduce((acc, div) => acc + div.teams.length, 0)
  ...
  return (
    <div className="border rounded-lg">
      ...
      <div className="flex items-center justify-between p-4 bg-muted/50">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={onAddDivision}>
            <Plus className="h-4 w-4 mr-2" />
            Add Division
          </Button>
        ...
        <div className="p-4 pt-0">
          {conference.divisions.length === 0 ? (
            <div className="text-center py-8 border-2 border-dashed border-muted rounded-lg">
              <p className="text-muted-foreground mb-2">No divisions in this conference</p>
              <Button variant="outline" size="sm" onClick={onAddDivision}>
                <Plus className="h-4 w-4 mr-2" />
                Add First Division
              </Button>
            </div>
            ...
            <div className="space-y-3">
              {conference.divisions.map((division) => (
                <DivisionTreeItem key={division.id} division={division} />
              ))}
            </div>
          )}
        </div>
...
function ConferenceForm({ seasonId, onClose }: { seasonId: string; onClose: () => void }) {
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData((prev) => ({ ...prev, minTeams: e.target.value }))}
            placeholder="4"
            min="1"
          />
        </div>
...
function SessionScheduleCard({ session, games }: { session: any; games: AdminGame[] }) {
  ...
  return (
    <div className="border rounded-lg">
      <div className="flex items-center justify-between p-4 bg-muted/50">
        ...
        <div className="flex items-center gap-2">
          <Badge variant={session.type === "Regular" ? "default" : "secondary"}>{session.type}</Badge>
          <Button variant="ghost" size="sm">
            <Edit className="h-4 w-4" />
          </Button>
        </div>
      ...
      {isExpanded && (
        <div className="p-4 pt-0">
          {games.length === 0 ? (
            <div className="text-center py-8 border-2 border-dashed border-muted rounded-lg">
              <p className="text-muted-foreground">No games scheduled for this session</p>
            </div>
          ) : (
            <div className="space-y-3">
              {games.map((game) => (
    ...
    <div className="flex items-center justify-between p-3 border rounded-lg bg-background">
      <div className="flex items-center gap-4">
        <div className="text-center">
          <div className="text-sm font-medium">{new Date(game.dateTime).toLocaleDateString()}</div>
          <div className="text-xs text-muted-foreground">{new Date(game.dateTime).toLocaleTimeString()}</div>
        </div>
        <div className="flex items-center gap-2">
...
function GameForm({ seasonId, onClose }: { seasonId: string; onClose: () => void }) {
  const season = getAdminSeasonById(seasonId)
  const sessions = getAdminSessionsBySeasonId(seasonId)
  const [formData, setFormData] = useState({
    sessionId: "",
