...
import { ArrowLeft, Plus, Edit, Trash2, Users, UserPlus, Settings, Trophy, Calendar } from "lucide-react"
import { getDivisionById, getTeamsByDivision, adminSampleData, type AdminTeam } from "@/lib/admin-data"
...
export function DivisionDetail({ divisionId }: { divisionId: string }) {
      ...
      <div className="container mx-auto px-4 py-8">
        <Card>
          <CardContent className="p-12 text-center">
            <h2 className="text-2xl font-bold text-foreground mb-2">Division Not Found</h2>
            <p className="text-muted-foreground mb-4">The requested division could not be found.</p>
            <Button asChild>
  ...
  const availableTeams = adminSampleData.teams.filter((team) => !team.divisionId || team.divisionId !== divisionId)
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="text-4xl font-bold text-foreground mb-2">{division.name}</h1>
          <p className="text-muted-foreground">{division.description || "Division management"}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-foreground">{assignedTeams.length}</div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-foreground">
              {division.maxTeams ? division.maxTeams - assignedTeams.length : "∞"}
            </div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-foreground">{availableTeams.length}</div>
          ...
          <div className="flex gap-2">
            ...
            <Dialog open={isTeamDialogOpen} onOpenChange={setIsTeamDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                ...
                <TeamForm divisionId={divisionId} onClose={() => setIsTeamDialogOpen(false)} />
              </DialogContent>
            </Dialog>
          </div>
        </CardHeader>
        <CardContent>
          {assignedTeams.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-foreground mb-2">No teams assigned</h3>
              <p className="text-muted-foreground mb-4">Register new teams or assign existing teams to this division</p>
            ...
            <div className="space-y-4">
              {assignedTeams.map((team) => (
                <TeamCard key={team.id} team={team} divisionId={divisionId} />
              ))}
            </div>
          )}
        </CardContent>
        ...
        <Card className="mt-6 border-orange-200 bg-orange-50">
          <CardContent className="p-4">
            ...
            <p className="text-sm text-orange-700 mt-1">
              This division has reached its maximum capacity of {division.maxTeams} teams.
            </p>
          </CardContent>
        </Card>
      )}
...
function TeamCard({ team, divisionId }: { team: AdminTeam; divisionId: string }) {
  const primaryCoach = team.coaches.find((coach) => coach.role === "head") || team.coaches[0]
  const primaryContact = team.contacts.find((contact) => contact.isPrimary) || team.contacts[0]
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-2 pt-4">
        ...
        <Button type="submit" disabled={!selectedTeamId} className="flex-1 bg-primary hover:bg-primary/90">
          Assign Team
        </Button>
      </div>
    </form>
...
function TeamForm({ divisionId, onClose }: { divisionId: string; onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="home-venue" className="block text-sm font-medium mb-1">
            Home Venue
          </label>
          <input
            id="home-venue"
            ...
            onChange={(e) => setFormData((prev) => ({ ...prev, homeVenue: e.target.value }))}
            className="w-full px-3 py-2 border border-input rounded-md"
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="coach-name" className="block text-sm font-medium mb-1">
              Coach Name *
            </label>
            <input
              id="coach-name"
          ...
          <div>
            <label htmlFor="coach-email" className="block text-sm font-medium mb-1">
              Coach Email
            </label>
            <input
              id="coach-email"
