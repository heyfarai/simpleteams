...
import { Search, Users, Trophy, Filter, X, Grid3X3, BarChart3, Check, XIcon, Minus } from "lucide-react"
import { sampleTeams, getPlayersByTeam, sampleSeasons, sampleSessions } from "@/lib/sample-data"
...
export function TeamsDirectory() {
  ...
  const divisions = ["all", ...Array.from(new Set(sampleTeams.map((team) => team.division)))]
  const years = ["all", ...Array.from(new Set(sampleSeasons.map((season) => season.year.toString())))]
  const sessions = ["all", ...Array.from(new Set(sampleSessions.map((session) => session.name)))]
  const allAwards = Array.from(new Set(sampleTeams.flatMap((team) => team.awards)))
  ...
  const filteredTeams = teams.filter((team) => {
    ...
    const matchesAwards = selectedAwards.length === 0 || selectedAwards.some((award) => team.awards.includes(award))

    return matchesSearch && matchesDivision && matchesYear && matchesSession && matchesAwards
  })

  const handleAwardToggle = (award: string) => {
    setSelectedAwards((prev) => (prev.includes(award) ? prev.filter((a) => a !== award) : [...prev, award]))
  }
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {allAwards.map((award) => (
            <div key={award} className="flex items-center space-x-2">
              ...
              />
              <label htmlFor={`award-${award}`} className="text-sm text-foreground cursor-pointer flex-1">
                {award}
              </label>
            </div>
          ))}
  ...
  const getStandingsData = () => {
    const teamStats = filteredTeams.map((team) => {
      ...
      const ppg = stats.gamesPlayed > 0 ? stats.pointsFor / stats.gamesPlayed : 0
      const oppPpg = stats.gamesPlayed > 0 ? stats.pointsAgainst / stats.gamesPlayed : 0
    ...
    const divisions = Array.from(new Set(teamStats.map((team) => team.division)))
    return divisions.map((division) => ({
      division,
      teams: teamStats.filter((team) => team.division === division).sort((a, b) => b.winPercentage - a.winPercentage),
    }))
  }
    ...
    <div className="flex gap-6">
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-foreground">Teams ({filteredTeams.length})</h2>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {selectedAwards.map((award) => (
              <Badge key={award} variant="secondary" className="gap-1">
                {award}
                <X className="h-3 w-3 cursor-pointer" onClick={() => handleAwardToggle(award)} />
              </Badge>
            ))}
          </div>
        )}
                ...
                <Card className="group hover:shadow-lg transition-shadow duration-300 cursor-pointer">
                  <CardContent className="p-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                      ...
                      <div>
                        <div className="text-lg font-bold text-primary">{team.playerCount}</div>
                          ...
                          {teams.map((team, index) => (
                            <tr
                              ...
                              className={`border-b hover:bg-muted/30 transition-colors ${
                                index < playoffCutoff ? "bg-green-50 dark:bg-green-950/20" : ""
                              }`}
                            >
                              <td className="p-4">
                                <div className="flex items-center gap-2">
                              ...
                              <td className="p-4">
                                ...
                                >
                                  ...
                                  <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">{team.coach}</div>
                                  </div>
                                </Link>
                              </td>
                              <td className="p-4 text-center font-semibold text-green-600">{team.stats.wins}</td>
                              <td className="p-4 text-center font-semibold text-red-600">{team.stats.losses}</td>
                              <td className="p-4 text-center font-semibold">{team.winPercentage.toFixed(1)}%</td>
                              <td className="p-4 text-center">{team.ppg.toFixed(1)}</td>
                              <td className="p-4 text-center">{team.oppPpg.toFixed(1)}</td>
                              <td
                                className={`p-4 text-center font-semibold ${team.pointsDiff >= 0 ? "text-green-600" : "text-red-600"}`}
                              >
                                {team.pointsDiff >= 0 ? "+" : ""}
